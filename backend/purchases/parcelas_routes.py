...
def listar_contas(
    ...
    user: UserContext = Depends(require_permission("contas_pagar:ver"))
):
    """Lista contas a pagar."""
    resultado = list_contas_a_pagar(ensure_company_access(user, None), status, fornecedorId)
...
def listar_vencendo(
    ...
    resultado = get_parcelas_vencendo(ensure_company_access(user, None), dias)
    return {"data": resultado}
...
def listar_atrasadas(
    ...
    resultado = get_parcelas_atrasadas(ensure_company_access(user, None))
    return {"data": resultado}
...
def obter_conta(
    ...
    resultado = get_conta_a_pagar(ensure_company_access(user, None), contaId)
    return {"data": resultado}
...
def realizar_baixa_parcela(
    ...
    contaId: str = Path(...),
    parcelaNum: int = Path(...),
    user: UserContext = Depends(require_permission("contas_pagar:baixar"))
):
